...
function FuseNavVerticalItem({item, classes, nestedLevel, userRole, navbarCloseMobile, active, rolePath, translate})
{
    if ( item.auth && (!item.auth.includes(userRole) || (userRole !== 'guest' && item.auth.length === 1 && item.auth.includes('guest'))) )
    {
        return null;
    }
	if(rolePath){ //ignore item
		const roleMatch = rolePath.find(route => item.url.startsWith(route));
		if(!roleMatch)
			return null;
	}
...
export default withTranslate(NavVerticalItem);
