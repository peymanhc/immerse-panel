...
class FuseNavVerticalCollapse extends Component
{
    ...
    render()
    {
        const {item, nestedLevel, classes, userRole, active, rolePath, translate} = this.props;
        if ( item.auth && (!item.auth.includes(userRole) || (userRole !== 'guest' && item.auth.length === 1 && item.auth.includes('guest'))) )
        {
            return null;
        }
		if(rolePath && item.children){ //ignore empty collapse
			const allUrls = item.children.filter(({type}) => type === "item").map(({url}) => url);
			const childrenUrl = item.children.filter(({type}) => type === "collapse").map(({children}) => children).flat().map(({url}) => url);
			const match = [...allUrls, ...childrenUrl].find(url => rolePath.find(route => url.startsWith(route)));
			if(!match)
				return null;
		}		
        ...
        return (
            <ul className={classNames(classes.root, this.state.open && "open")}>
                ...
                >
                    ...
                    <ListItemText className="list-item-text" primary={translate(item.title)} classes={{primary: 'text-14'}}/>
                    {item.badge && (
...
const NavVerticalCollapse = withStyles(styles, {withTheme: true})(withRouter(connect(mapStateToProps)(withTranslate(FuseNavVerticalCollapse))));
