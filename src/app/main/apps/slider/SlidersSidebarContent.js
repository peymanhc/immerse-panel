...
function SlidersSidebarContent({classes, openNewLabelDialog})
{
	const folder = {
        'id'    : 0,
        'handle': 'all',
                    ...
                    <List>
                        ...
                        {filters.length > 0 && filters.map((filter) => (
                            <ListItem
                                button
                                component={NavLink}
                                to={'/apps/sliders/labels/' + filter.handle}
                                activeClassName="active"
                                className={classes.listItem}
                                key={filter.id}
                            >
...
function mapDispatchToProps(dispatch)
{
    return bindActionCreators({
        openNewLabelDialog: Actions.openNewSlidersDialog
    }, dispatch);
}
...
export default withStyles(styles, {withTheme: true})(withRouter(connect(mapStateToProps, mapDispatchToProps)(SlidersSidebarContent)));
