...
function PropertyLabelsSidebarContent({classes, openNewLabelDialog, translate})
{
	const folder = {
        'id'    : 0,
        'handle': 'all',
                    ...
                    <List>
                        ...
                        {filters.length > 0 && filters.map((filter) => (
                            <ListItem
                                button
                                component={NavLink}
                                to={'/apps/property/labels/' + filter.handle}
                                activeClassName="active"
                                className={classes.listItem}
                                key={filter.id}
                            >
...
export default withStyles(styles, {withTheme: true})(withRouter(connect(mapStateToProps, mapDispatchToProps)(withTranslate(PropertyLabelsSidebarContent))));
