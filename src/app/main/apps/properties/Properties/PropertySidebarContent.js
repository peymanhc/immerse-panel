...
function PropertySidebarContent({classes, folders, filters, labels, openNewLabelDialog, translate})
{
	const folder = {
        'id'    : 0,
        'handle': 'all',
                ...
                <div className={classes.listWrapper}>
                    <List>
                        <ListItem
                            button
                            component={NavLink}
                            to={'/apps/properties/' + folder.handle} key={folder.id}
                    ...
                    <List>
                        ...
                        {filters.length > 0 && filters.map((filter) => (
                            <ListItem
                                button
                                component={NavLink}
                                to={'/apps/properties/filter/' + filter.handle}
                                activeClassName="active"
                                className={classes.listItem}
                                key={filter.id}
                            >
                    ...
                    <List>
                        ...
                        {labels.length > 0 && labels.map((label) => (
                            <ListItem
                                button
                                component={NavLink}
                                to={'/apps/properties/label/' + label.handle}
                                key={label.id}
                                className={classes.listItem}
                            >
...
{
    return {
        folders: PropertyApp.folders,
        labels : PropertyApp.labels,
        filters: PropertyApp.filters
    }
}

export default withStyles(styles, {withTheme: true})(withRouter(connect(mapStateToProps, mapDispatchToProps)(withTranslate(PropertySidebarContent))));
