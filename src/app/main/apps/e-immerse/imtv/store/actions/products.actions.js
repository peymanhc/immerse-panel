...
{	
    ...
    const request = axios.get('/api/imtv-app/products', {
        params: {
			limit: perPage,
			skip: skip ,
			type: 'imtv',
...
{
	const skip = (perPage * (page + 1)) - perPage;
	const request = axios.delete('/api/imtv-app/products/remove', {data : {idList, limit: perPage, skip}});
    return (dispatch) =>
        request.then((response) =>
            dispatch({
                type   : GET_PRODUCTS,
...
const uploadExcel = (file) => {	
	return new Promise((resolve, reject) => {
		...
		axios.post('/api/imtv-app/excel-upload', formData).then(({data}) => resolve(data)).catch((e) => reject(e));	 			
	});
};
