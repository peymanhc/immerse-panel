                ...
                <div className={classes.listWrapper}>
                    <List>
                        <ListItem
                            button
                            component={NavLink}
                            to={'/apps/e-immerse/imtv-categories/' + folder.handle} key={folder.id}
                    ...
                    <List>
                        ...
                        {filters.length > 0 && filters.map((filter) => (
                            <ListItem
                                button
                                component={NavLink}
                                to={'/apps/e-immerse/imtv-categories/filter/' + filter.handle}
                                activeClassName="active"
                                className={classes.listItem}
                                key={filter.id}
                            >
                    ...
                    <List>
                        ...
                        {labels.length > 0 && labels.map((label) => (
                            <ListItem
                                button
                                component={NavLink}
                                to={'/apps/e-immerse/imtv-categories/label/' + label.handle}
                                key={label.id}
                                className={classes.listItem}
                            >
...
{
    return {
        folders: imtvCategoryApp.folders,
        labels : imtvCategoryApp.labels,
        filters: imtvCategoryApp.filters
    }
}
