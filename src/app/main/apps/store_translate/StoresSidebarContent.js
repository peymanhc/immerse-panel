...
function StoresSidebarContent({classes, labels})
{
    return (
                    ...
                    <List>
                        ...
                        {labels.length > 0 && labels.map((filter) => (
                            <ListItem
                                button
                                component={NavLink}
                                to={'/apps/stores/labels/' + filter.title}
...
{  
    return {
		labels : storesApp.stores.labels,
    }
}

export default withStyles(styles, {withTheme: true})(withRouter(connect(mapStateToProps)(StoresSidebarContent)));
