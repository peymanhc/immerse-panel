...
class AddUserDialog extends Component {
    ...
    render()
    { 
        const {classes, warrantyAddUserDialog, closeAddUserDialog, users, addresses, addDataToWarranty, translate} = this.props; 
        const {userId, addressId} = this.state; 
        ...
        return (
            <Dialog
				open={warrantyAddUserDialog.open}
                classes={{
                    paper: "m-24"
                }}
            ...
            >
                <AppBar position="static" elevation={1}>
                    <Toolbar className="block w-full p-24">
						...
						<div>
							<CircularProgress 
								className={warrantyAddUserDialog.loading? classes.loading:classes.hidden} 
								color="secondary"
							/>
						</div>						
                    </Toolbar>
                ...
                <DialogContent classes={{root: "p-24"}} style={{minHeight:250}}>
                    ...
                    <DialogActions className="justify-between pl-16">
                        <Button
                            ...
                            color="primary"
                            onClick={() => {
								addDataToWarranty({'customer':user, invoiceAddress:[address]});
								closeAddUserDialog();
							}}
...
{ 
    return {
		warrantyAddUserDialog	: eCommerceApp.newWarranty.warrantyAddUserDialog,
		users				: eCommerceApp.newWarranty.users,
		addresses			: eCommerceApp.newWarranty.addresses,
    }
}
