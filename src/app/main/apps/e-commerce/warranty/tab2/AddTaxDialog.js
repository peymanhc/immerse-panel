...
class AddTaxDialog extends Component {
	...
	componentDidUpdate(prevProps, prevState, snapshot){
        if (prevProps.warrantyAddTaxDialog.open === false && this.props.warrantyAddTaxDialog.open )
        {
			const tax = this.props.warrantyAddTaxDialog.data;
    ...
    render()
    {  
        const {classes, warrantyAddTaxDialog, closeAddTaxDialog, tax} = this.props;
        ...
        return (
            <Dialog
				open={warrantyAddTaxDialog.open}
                classes={{
                    paper: "m-24"
                }}
            ...
            >
                <AppBar position="static" elevation={1}>
                    <Toolbar className="block w-full p-24">
						...
						<div>
							<CircularProgress 
								className={warrantyAddTaxDialog.loading? classes.loading:classes.hidden} 
								color="secondary"
							/>
						</div>						
                    </Toolbar>
                ...
                <DialogContent classes={{root: "p-24"}} style={{minHeight:250}}>
                    ...
                    <DialogActions className="justify-between pl-16">
                        ...
                        >
						{warrantyAddTaxDialog.data === null ? 'Add' : 'Save'}
                        </Button>
                    </DialogActions>
            </Dialog>
        );
...
{ 
    return {
		warrantyAddTaxDialog	: eCommerceApp.newWarranty.warrantyAddTaxDialog,
		tax					: eCommerceApp.newWarranty.tax,
    }
}
