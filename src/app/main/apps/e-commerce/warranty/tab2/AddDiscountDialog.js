...
class AddDiscountDialog extends Component {
	...
	componentDidUpdate(prevProps, prevState, snapshot){
        if (prevProps.warrantyAddDiscountDialog.open === false && this.props.warrantyAddDiscountDialog.open )
        {
			const discount = this.props.warrantyAddDiscountDialog.data;
    ...
    render()
    {  
        const {classes, warrantyAddDiscountDialog, closeAddDiscountDialog, discounts} = this.props;
        ...
        return (
            <Dialog
				open={warrantyAddDiscountDialog.open}
                classes={{
                    paper: "m-24"
                }}
            ...
            >
                <AppBar position="static" elevation={1}>
                    <Toolbar className="block w-full p-24">
						...
						<div>
							<CircularProgress 
								className={warrantyAddDiscountDialog.loading? classes.loading:classes.hidden} 
								color="secondary"
							/>
						</div>						
                    </Toolbar>
                ...
                <DialogContent classes={{root: "p-24"}} style={{minHeight:250}}>
                    ...
                    <DialogActions className="justify-between pl-16">
                        ...
                        >
						{warrantyAddDiscountDialog.data === null ? 'Add' : 'Save'}
                        </Button>
                    </DialogActions>
            </Dialog>
        );
...
{ 
    return {
		warrantyAddDiscountDialog	: eCommerceApp.newWarranty.warrantyAddDiscountDialog,
		discounts				: eCommerceApp.newWarranty.discounts,
    }
}
