...
import {sendMessage} from '../store/actions/warranty.actions'; 
...
class Chat extends Component {
    ...
    onMessageSubmit = (ev) => {
        ...
        this.props.sendMessage(this.state.messageText, this.props.warrantyId)
            .then(() => {
    ...
    render()
    {
        const {classes, chat, user, className, customer, warrantyId, translate} = this.props;
        ...
        return (
            <div className={classNames("flex flex-col relative", className)}>
                ...
                {chat && (
                    <form onSubmit={this.onMessageSubmit} className="absolute pin-b pin-r pin-l py-16 px-8" disabled={warrantyId ? false : true}>
                        <Paper className="flex items-center relative rounded-24" elevation={1}>
                            <TextField
                                ...
                                value={messageText}
								disabled={warrantyId ? false : true}
                            />
                            <IconButton className="absolute pin-r pin-t" type="submit" disabled={warrantyId ? false : true}>
