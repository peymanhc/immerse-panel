...
function WarrantylistStatusSidebarContent({classes, openNewLabelDialog, translate})
{
	const folder = {
        'id'    : 0,
        'handle': 'all',
            ...
            <div className="flex-auto border-l-1 border-solid">
                ...
                <div className="p-24">
                    <Button
                        ...
                        className="w-full"
                    >
					{translate('ADD_Warrantylist_Status')}
                    </Button>
                </div>
                ...
                <div className={classes.listWrapper}>
                    <List>
                        <ListItem
                            button
                            component={NavLink}
                            to={'/apps/e-commerce/warrantylist-status/labels/all'} 
                    ...
                    <List>
                        ...
                        {filters.length > 0 && filters.map((filter) => (
                            <ListItem
                                button
                                component={NavLink}
                                to={'/apps/e-commerce/warrantylist-status/labels/' + filter.handle}
                                activeClassName="active"
                                className={classes.listItem}
                                key={filter.id}
                            >
...
function mapDispatchToProps(dispatch)
{
    return bindActionCreators({
        openNewLabelDialog: Actions.openNewWarrantylistStatusDialog
    }, dispatch);
}
...
export default withStyles(styles, {withTheme: true})(withRouter(connect(mapStateToProps, mapDispatchToProps)(withTranslate(WarrantylistStatusSidebarContent))));