...
class AddUserDialog extends Component {
    ...
    render()
    { 
        const {classes, orderAddUserDialog, closeAddUserDialog, users, addresses, addDataToOrder, translate} = this.props; 
        const {userId, addressId} = this.state; 
            ...
            >
                <AppBar position="static" elevation={1}>
                    <Toolbar className="block w-full p-24">
						...
						<div>
							<CircularProgress 
								className={orderAddUserDialog.loading? classes.loading:classes.hidden} 
								color="secondary"
							/>
						</div>						
                    </Toolbar>
...
{ 
    return {
		orderAddUserDialog	: eCommerceApp.newOrder.orderAddUserDialog,
		users				: eCommerceApp.newOrder.users, 
		addresses			: eCommerceApp.newOrder.addresses,
    }
}
