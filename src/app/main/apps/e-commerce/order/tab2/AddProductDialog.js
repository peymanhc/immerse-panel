...
class AddProductDialog extends Component {
	...
	componentDidUpdate(prevProps, prevState, snapshot){
        if (prevProps.orderAddProductDialog.open === false && this.props.orderAddProductDialog.open )
        {
			const product = this.props.orderAddProductDialog.data;
	...
	addProduct = () => {
		...
		const data = this.props.orderAddProductDialog.data;
    ...
    render()
    {  
        const {classes, orderAddProductDialog, closeAddProductDialog, products , translate } = this.props; 
        ...
        return (
            <Dialog
				open={orderAddProductDialog.open}
                classes={{
                    paper: "m-24"
                }}
            ...
            >
                <AppBar position="static" elevation={1}>
                    <Toolbar className="block w-full p-24">
						...
						<div>
							<CircularProgress 
								className={orderAddProductDialog.loading? classes.loading:classes.hidden} 
								color="secondary"
							/>
						</div>						
                    </Toolbar>
                ...
                <DialogContent classes={{root: "p-24"}} style={{minHeight:350}}>
                    ...
                    <DialogActions className="justify-between pl-16">
                        ...
                        >
						{orderAddProductDialog.data === null ? 'Add' : 'Save'}
                        </Button>
                    </DialogActions>
            </Dialog>
        );
...
{ 
    return {
		orderAddProductDialog	: eCommerceApp.newOrder.orderAddProductDialog,
		products				: eCommerceApp.newOrder.products,
    }
}
