...
const OrderChat = ({order, user, classes}) => {
	const {customer, chat} = order;	
    return customer ? (
		<main className={classNames(classes.contentWrapper, "z-10")}>
			<div className={classes.content}>
				<Chat customer={customer} user={user} chat={{dialog:chat}} selectedContactId={customer.id} orderId={order.id} className="flex flex-1 z-10" />
			</div>
		</main>	
    ):(<></>);
...
export default withStyles(styles)(connect(mapStateToProps)(OrderChat));
