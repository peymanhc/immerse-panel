...
class OrderAddStatusDialog extends Component {
	...
	addStatus = () => {
        const {orderId, addOrderToStatus} = this.props; 
		const { statusId, description, imgSrc1, imgSrc2} = this.state;		
		addOrderToStatus(orderId, statusId, imgSrc1, imgSrc2, description).then(() => {
    ...
    render()
    {
        const {orderStatus, orderAddStatusDialog, translate, classes} = this.props; 
		const { statusId, description, imgSrc1, imgSrc2} = this.state;
		
		const allStatus = orderStatus.map(({name: label, id: value}) => ({value, label}));
            ...
            >
                <AppBar position="static" elevation={1}>
                    <Toolbar className="flex w-full">
						...
						<div className="flex-grow">
							<CircularProgress 
								className={this.props.statusOrderImageLoading? classes.loading:classes.hidden} 
								color="secondary"
							/>
						</div>							
                    </Toolbar>					
...
function mapDispatchToProps(dispatch)
{
    return bindActionCreators({
        ...
        addOrderToStatus	: Actions.addOrderToStatus,
    }, dispatch);
}
...
{ 
    return {
		orderStatus				: eCommerceApp.order.orderStatus,
		orderAddStatusDialog	: eCommerceApp.order.orderAddStatusDialog,
		statusOrderImageLoading	: eCommerceApp.order.statusOrderImageLoading,
    }
}

export default withStyles(styles, {withTheme: true})(connect(mapStateToProps, mapDispatchToProps)(withTranslate(OrderAddStatusDialog)));
