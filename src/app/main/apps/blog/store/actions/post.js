...
{
    const request = axios.get('/api/blog-app/category-labels');
...
{
    const request = axios.get('/api/blog-app/types');
...
{
    const request = axios.get('/api/blog-app/property-labels');
...
{
    const request = axios.get('/api/blog-app/categories');
...
{
    const request = axios.get('/api/blog-app/properties');
...
export function getPost(params) {
    const request = axios.get('/api/blog-app/post', {params});
	...
	return (dispatch) => {
		...
		const uploadImage = (file, id) => {	
			return new Promise((resolve, reject) => {
				...
				if(postId === 'new')
					axios.post('/api/blog-app/upload', formData).then(({data}) => resolve(data)).catch((e) => reject(e));	 
				else
					axios.put('/api/blog-app/upload', formData).then(({data}) => resolve(data)).catch((e) => reject(e));	 			
			});
		};
		...
		return saveImage(images).then((images) => {
			...
			form.type = "blog";
			if(postId === 'new')
				request = axios.post('/api/blog-app/post/create', form);		
			else
				request = axios.put('/api/blog-app/post/save', form);
