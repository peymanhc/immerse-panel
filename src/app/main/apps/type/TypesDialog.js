...
class TypesDialog extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot)
    {
         ...
         * After Dialog Open
         */
        if ( !prevProps.typesDialog.props.open && this.props.typesDialog.props.open )
        {
            /**
             * Dialog type: 'edit'
             * Update State
             */
            if ( this.props.typesDialog.type === 'edit' &&
                this.props.typesDialog.data &&
                !_.isEqual(this.props.typesDialog.data, prevState) )
            {
                this.setState({form: {...this.props.typesDialog.data}});
            }
             ...
             * Update State
             */
            if ( this.props.typesDialog.type === 'new' &&
                !_.isEqual(newTypesState, prevState) )
            {
                this.setState({
                    form: {
                        ...newTypesState,
    ...
    closeTypesDialog = () => {
        this.props.typesDialog.type === 'edit' ? this.props.closeEditTypesDialog() : this.props.closeNewTypesDialog();
    };
    ...
    render()
    {  
        const {typesDialog, addTypes, updateTypes, removeTypes, classes} = this.props;
        const {form} = this.state;

        return (
            <Dialog {...typesDialog.props} onClose={this.closeTypesDialog} fullWidth maxWidth="sm">

                <AppBar position="static" elevation={1}>
                    <Toolbar className="flex w-full">
                        <Typography variant="subtitle1" color="inherit">
                            {typesDialog.type === 'new' ? 'New Type' : 'Edit Type'}
                        </Typography>
						<div>
							<CircularProgress 
                ...
                {typesDialog.type === 'new' ? (
                    <DialogActions className="justify-between pl-8 sm:pl-16">
                        <Button
                            variant="contained"
                            color="primary"
                            onClick={() => {
                                addTypes(this.state.form);
                            }}
                    ...
                    <DialogActions className="justify-between pl-8 sm:pl-16">
                        ...
                        <IconButton
                            ...
                            onClick={() => {
                                removeTypes(this.state.form.id);
...
function mapDispatchToProps(dispatch)
{
    return bindActionCreators({
        closeEditTypesDialog: Actions.closeEditTypesDialog,
        closeNewTypesDialog : Actions.closeNewTypesDialog,
        addTypes            : Actions.addTypes,
        updateTypes         : Actions.updateTypes,
        removeTypes         : Actions.removeTypes
    }, dispatch);
}
...
{
    return { 
        typesDialog		: typesApp.types.typesDialog,
		loading				: typesApp.types.loading,
    }
}

export default withStyles(styles, {withTheme: true})(connect(mapStateToProps, mapDispatchToProps)(TypesDialog));
