...
function DictionarysSidebarContent({classes, openNewLabelDialog})
{
	const folder = {
        'id'    : 0,
        'handle': 'all',
                ...
                <div className={classes.listWrapper}>
                    <List>
                        <ListItem
                            button
                            component={NavLink}
                            to={'/apps/dictionarys/labels/all'} 
                    ...
                    <List>
                        ...
                        {filters.length > 0 && filters.map((filter) => (
                            <ListItem
                                button
                                component={NavLink}
                                to={'/apps/dictionarys/labels/' + filter.handle}
                                activeClassName="active"
                                className={classes.listItem}
                                key={filter.id}
                            >
...
function mapDispatchToProps(dispatch)
{
    return bindActionCreators({
          openNewLabelDialog: Actions.openNewDictionarysDialog
    }, dispatch);
}
...
export default withStyles(styles, {withTheme: true})(withRouter(connect(mapStateToProps, mapDispatchToProps)(DictionarysSidebarContent)));
