...
class AddProductDialog extends Component {
	...
	componentDidUpdate(prevProps, prevState, snapshot){
        if (prevProps.accdocAddProductDialog.open === false && this.props.accdocAddProductDialog.open )
        {
			const product = this.props.accdocAddProductDialog.data;
	...
	addProduct = () => {
		...
		const data = this.props.accdocAddProductDialog.data;
    ...
    render()
    {  
        const {classes, accdocAddProductDialog, closeAddProductDialog, products , translate } = this.props; 
        ...
        return (
            <Dialog
				open={accdocAddProductDialog.open}
                classes={{
                    paper: "m-24"
                }}
            ...
            >
                <AppBar position="static" elevation={1}>
                    <Toolbar className="block w-full p-24">
						...
						<div>
							<CircularProgress 
								className={accdocAddProductDialog.loading? classes.loading:classes.hidden} 
								color="secondary"
							/>
						</div>						
                    </Toolbar>
                ...
                <DialogContent classes={{root: "p-24"}} style={{minHeight:350}}>
                    ...
                    <DialogActions className="justify-between pl-16">
                        ...
                        >
						{accdocAddProductDialog.data === null ? 'Add' : 'Save'}
                        </Button>
                    </DialogActions>
            </Dialog>
        );
...
{ 
    return {
		accdocAddProductDialog	: eAccountApp.newAccdoc.accdocAddProductDialog,
		products				: eAccountApp.newAccdoc.products,
    }
}
