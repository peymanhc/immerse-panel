...
class AddDiscountDialog extends Component {
	...
	componentDidUpdate(prevProps, prevState, snapshot){
        if (prevProps.accdocAddDiscountDialog.open === false && this.props.accdocAddDiscountDialog.open )
        {
			const discount = this.props.accdocAddDiscountDialog.data;
    ...
    render()
    {  
        const {classes, accdocAddDiscountDialog, closeAddDiscountDialog, discounts} = this.props;
        ...
        return (
            <Dialog
				open={accdocAddDiscountDialog.open}
                classes={{
                    paper: "m-24"
                }}
            ...
            >
                <AppBar position="static" elevation={1}>
                    <Toolbar className="block w-full p-24">
						...
						<div>
							<CircularProgress 
								className={accdocAddDiscountDialog.loading? classes.loading:classes.hidden} 
								color="secondary"
							/>
						</div>						
                    </Toolbar>
                ...
                <DialogContent classes={{root: "p-24"}} style={{minHeight:250}}>
                    ...
                    <DialogActions className="justify-between pl-16">
                        ...
                        >
						{accdocAddDiscountDialog.data === null ? 'Add' : 'Save'}
                        </Button>
                    </DialogActions>
            </Dialog>
        );
...
{ 
    return {
		accdocAddDiscountDialog	: eAccountApp.newAccdoc.accdocAddDiscountDialog,
		discounts				: eAccountApp.newAccdoc.discounts,
    }
}
