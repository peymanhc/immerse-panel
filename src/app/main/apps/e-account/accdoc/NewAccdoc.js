...
import {Icon, Tab, Tabs, Typography, Button} from '@material-ui/core';
...
class NewOrder extends Component {
    ...
    componentDidMount()
    {
		const {accdocId} = this.props.match.params;
		if(accdocId === "new")
			this.setState({order:newAccdoc});
		this.props.getData();
    }
	...
	addOrder = () => {
		...
		this.props.createOrder(order).then((data) => {
			this.props.history.push('/apps/e-account/accdocs/' + data.id);
		});
	};
...
export default withReducer('eAccountApp', reducer)(withRouter(connect(null, mapDispatchToProps)(withTranslate(NewOrder))));
