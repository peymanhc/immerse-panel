...
function AccdocsStatusSidebarContent({classes, openNewLabelDialog, translate})
{
	const folder = {
        'id'    : 0,
        'handle': 'all',
                ...
                <div className={classes.listWrapper}>
                    <List>
                        <ListItem
                            button
                            component={NavLink}
                            to={'/apps/e-account/accdocs-status/labels/all'} 
                    ...
                    <List>
                        ...
                        {filters.length > 0 && filters.map((filter) => (
                            <ListItem
                                button
                                component={NavLink}
                                to={'/apps/e-account/accdocs-status/labels/' + filter.handle}
                                activeClassName="active"
                                className={classes.listItem}
                                key={filter.id}
                            >
...
function mapDispatchToProps(dispatch)
{
    return bindActionCreators({
        openNewLabelDialog: Actions.openNewAccdocsStatusDialog
    }, dispatch);
}
...
export default withStyles(styles, {withTheme: true})(withRouter(connect(mapStateToProps, mapDispatchToProps)(withTranslate(AccdocsStatusSidebarContent))));