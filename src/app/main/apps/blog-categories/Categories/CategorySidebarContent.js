                ...
                <div className={classes.listWrapper}>
                    <List>
                        <ListItem
                            button
                            component={NavLink}
                            to={'/apps/blog-categories/' + folder.handle} key={folder.id}
                    ...
                    <List>
                        ...
                        {filters.length > 0 && filters.map((filter) => (
                            <ListItem
                                button
                                component={NavLink}
                                to={'/apps/blog-categories/filter/' + filter.handle}
                                activeClassName="active"
                                className={classes.listItem}
                                key={filter.id}
                            >
                    ...
                    <List>
                        ...
                        {labels.length > 0 && labels.map((label) => (
                            <ListItem
                                button
                                component={NavLink}
                                to={'/apps/blog-categories/label/' + label.handle}
                                key={label.id}
                                className={classes.listItem}
                            >
...
{
    return {
        folders: blogCategoryApp.folders,
        labels : blogCategoryApp.labels,
        filters: blogCategoryApp.filters
    }
}
