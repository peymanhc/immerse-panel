 ...
 ckEditorHandleChange = ( event, editor ) => {
    ...
    render()
    {
		...
		if(form && form.id && form.name)
			postLink = window.location.origin + "/tourism/post-full/" + form.id + "/" + form.name.split(' ').join('-');
		
  const adstypes = [
            {
                id:"1", title:"جاذبه های گردشگری",
            }, 
            {
                id:"2", title:"پتانسیل سرمایه گذاری",
            }, 
            {
                id:"3", title:"ایده کسب و کار",
            },
        ];
                        ...
                        <div className="flex flex-1 w-full items-center justify-between">
                            ...
                            <div className="flex flex-col items-start max-w-full">
                                ...
                                <FuseAnimate animation="transition.slideRightIn" delay={300}>
                                    <Typography className="normal-case flex items-center sm:mb-12" component={Link} role="button" to="/apps/tourism/posts">
...
function mapStateToProps({tourismApp})
{
    return {
        post: tourismApp.post
    }
}

export default 
	withReducer('tourismApp', reducer)(withStyles(styles, {withTheme: true})(withRouter(connect(mapStateToProps, mapDispatchToProps)(withTranslate(Post)))));
