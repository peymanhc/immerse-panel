...
{
    const request = axios.get('/api/notebook-app/category-labels');
...
{
    const request = axios.get('/api/notebook-app/types');
...
{
    const request = axios.get('/api/notebook-app/property-labels');
...
{
    const request = axios.get('/api/notebook-app/categories');
...
{
    const request = axios.get('/api/notebook-app/properties');
...
export function getPost(params) {
    const request = axios.get('/api/notebook-app/post', {params});
	...
	return (dispatch) => {
		...
		const uploadImage = (file, id) => {	
			return new Promise((resolve, reject) => {
				...
				if(postId === 'new')
					axios.post('/api/notebook-app/upload', formData).then(({data}) => resolve(data)).catch((e) => reject(e));	 
				else
					axios.put('/api/notebook-app/upload', formData).then(({data}) => resolve(data)).catch((e) => reject(e));	 			
			});
		};
		...
		return saveImage(images).then((images) => {
			...
			form.type = "notebook";
			if(postId === 'new')
				request = axios.post('/api/notebook-app/post/create', form);		
			else
				request = axios.put('/api/notebook-app/post/save', form);
