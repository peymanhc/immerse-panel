                ...
                <div className={classes.listWrapper}>
                    <List>
                        <ListItem
                            button
                            component={NavLink}
                            to={'/apps/news-categories/' + folder.handle} key={folder.id}
                    ...
                    <List>
                        ...
                        {filters.length > 0 && filters.map((filter) => (
                            <ListItem
                                button
                                component={NavLink}
                                to={'/apps/news-categories/filter/' + filter.handle}
                                activeClassName="active"
                                className={classes.listItem}
                                key={filter.id}
                            >
                    ...
                    <List>
                        ...
                        {labels.length > 0 && labels.map((label) => (
                            <ListItem
                                button
                                component={NavLink}
                                to={'/apps/news-categories/label/' + label.handle}
                                key={label.id}
                                className={classes.listItem}
                            >
...
function mapStateToProps({newsCategoryApp})
{
    return {
        folders: newsCategoryApp.folders,
        labels : newsCategoryApp.labels,
        filters: newsCategoryApp.filters
    }
}
