...
function ShopsSidebarContent({classes, labels})
{
    return (
                    ...
                    <List>
                        ...
                        {labels.length > 0 && labels.map((filter) => (
                            <ListItem
                                button
                                component={NavLink}
                                to={'/apps/shops/labels/' + filter.title}
...
{  
    return {
		labels : shopsApp.shops.labels,
    }
}

export default withStyles(styles, {withTheme: true})(withRouter(connect(mapStateToProps)(ShopsSidebarContent)));
