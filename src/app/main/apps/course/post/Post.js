...
import {withStyles, Button, Tab, Tabs, TextField, Icon, Typography, Checkbox, IconButton, FormControlLabel, Avatar, Chip } from '@material-ui/core';
...
class Post extends Component {
   ...
   handleEditorChange = (value) => {
    ...
    render()
    {
        const {classes, savePost, translate, post} = this.props;
        const {tabValue, form    } = this.state;
        const {companies, masters} = post;
		
		let		selectedMaster 		= undefined;
		let		selectedCompany 	= {name:"Freelancer", id: -1};
		const noneItem = {id: null, masterName: "None"};
		
		const newMasterList = [...masters, noneItem];
		
		const getImageList = () => [...form.images, ...this.props.post.files];
		let postLink = "";
		if(form && form.id && form.name)
			postLink = window.location.origin + "/course/post-full/" + form.id + "/" + form.name.split(' ').join('-');
		
		if(form){
			selectedMaster = newMasterList.find(({id}) => id === form.masterId);
			if(selectedMaster){
				if(selectedMaster.ownerIsFreelancer === false && selectedMaster.masterCompanyId !== null){
					let company = companies.find(item => item.id === selectedMaster.masterCompanyId);
					if(company)
						selectedCompany = {name:company.companyName, id: company.id};
				}
			}
		}
                        ...
                        <div className="flex flex-1 w-full items-center justify-between">
                            ...
                            <div className="flex flex-col items-start max-w-full">
                                ...
                                <FuseAnimate animation="transition.slideRightIn" delay={300}>
                                    <Typography className="normal-case flex items-center sm:mb-12" component={Link} role="button" to="/apps/course/posts">
                            ...
                            <FuseAnimate animation="transition.slideRightIn" delay={300}>
                                <Button
                                    ...
                                    onClick={() =>{
										...
										const {postId} = params;
										let newForm = {...form};
										if(selectedMaster && selectedMaster.id !== null){
											newForm.userId = selectedMaster.userId;
											if(selectedCompany)
												newForm.companyId = selectedCompany.id;											
										}
										else
											newForm = {...newForm, userId:null, companyId:-1, masterId:null};
										savePost({form:{...newForm}, images : this.props.post.files}, postId)
									}}
                                >
								{translate('Save')}
                                </Button>
                            ...
                            (
                                ...
                                <div className="flex">  
									<TextField
									 ...
									 /> 	<TextField
										...
										id="lat"
										name="lat"
										value={form.courseLocation  ? form.courseLocation[0] : ''}
										onChange={this.handleChange}
										variant="outlined" 	
										 InputLabelProps={{ className: classes.labelCustom,}} 									 
									 /> 	<TextField
										...
										id="long"
										name="long"
										value={form.courseLocation  ? form.courseLocation[1] : ''}
										onChange={this.handleChange}
										variant="outlined" 		
										 InputLabelProps={{ className: classes.labelCustom,}} 								 
									 /> 
                              ...
                              <div className="flex">  
                              	<FuseChipSelect
										...
										value={selectedMaster ? {value:selectedMaster.id, label:selectedMaster.masterName} : null}
...
function mapStateToProps({courseApp})
{
    return {
        post: courseApp.post
    }
...
export default 
	withReducer('courseApp', reducer)(withStyles(styles, {withTheme: true})(withRouter(connect(mapStateToProps, mapDispatchToProps)(withTranslate(Post)))));
