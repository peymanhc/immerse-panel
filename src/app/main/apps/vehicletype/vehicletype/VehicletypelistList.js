...
class VehicletypelistList extends Component{
	...
	render(){ 
		const {vehicletypelistCASE, searchText, orderBy, orderDescending} = this.props; 
		let arr = _.orderBy(this.getFilteredArray(vehicletypelistCASE, searchText), [orderBy], [orderDescending ? 'desc' : 'asc']);
				...
				<FuseAnimate delay={100}>
					<div className="flex flex-1 items-center justify-center h-full">
						<Typography color="textSecondary" variant="h5">
							There are no vehicletypelistCASE!
						</Typography>
					</div>
				</FuseAnimate>
			);
...
{
    return {
        vehicletypelistCASE	: vehicletypelistCASEApp.vehicletypelistCASE.entities,
        searchText     	: vehicletypelistCASEApp.vehicletypelistCASE.searchText,
        orderBy        	: vehicletypelistCASEApp.vehicletypelistCASE.orderBy,
        orderDescending	: vehicletypelistCASEApp.vehicletypelistCASE.orderDescending
    }
}
